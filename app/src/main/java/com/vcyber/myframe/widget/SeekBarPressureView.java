...
public class SeekBarPressureView extends View {
    ...
    private double defaultScreenHigh = 5;  //默认后滑块位置百分比

    private boolean isEdit = false;     //输入框是否正在输入
    
    private int allProcess = 5;

    public SeekBarPressureView(Context context) {
        this(context, null);
    }

    public SeekBarPressureView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SeekBarPressureView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
//        this.setBackgroundColor(Color.BLACK);

        Resources resources = getResources();
        notScrollBarBg = resources.getDrawable(R.mipmap.ic_b);
        hasScrollBarBg = resources.getDrawable(R.mipmap.ic_a);
        mThumbLow = resources.getDrawable(R.mipmap.ic_launcher);
        mThumbHigh = resources.getDrawable(R.mipmap.ic_launcher);
...
//        int height = measureHeight(heightMeasureSpec);
        ...
        mOffsetLow = formatDouble(defaultScreenLow / allProcess * (mDistance ))+ mThumbWidth / 2;
        mOffsetHigh = formatDouble(defaultScreenHigh / allProcess * (mDistance)) + mThumbWidth / 2;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        double progressLow = formatDouble((mOffsetLow - mThumbWidth / 2) * allProcess / mDistance);
        double progressHigh = formatDouble((mOffsetHigh - mThumbWidth / 2) * allProcess / mDistance);
...
//            }
    ...
    public void setProgressLow(double  progressLow) {
        ...
        mOffsetLow = formatDouble(progressLow / allProcess * (mDistance ))+ mThumbWidth / 2;
        isEdit = true;
        refresh();
    }
    ...
    public void setProgressHigh(double  progressHigh) {
        ...
        mOffsetHigh = formatDouble(progressHigh / allProcess * (mDistance)) + mThumbWidth / 2;
        isEdit = true;
        refresh();
    }
    ...
    public interface OnSeekBarChangeListener {
        ...
        //滑动时
        public void onProgressChanged(SeekBarPressureView seekBar, double progressLow,
                                      double progressHigh);
